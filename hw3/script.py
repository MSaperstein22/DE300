...
my_aws_access_key_id='ASIAYAAO5HRMBVUBB3CH'
my_aws_secret_access_key='fBd12lMpSPekTAPGy9bujaCkVfdhCy+hywDe6aMo'
my_aws_session_token=''
...
def train_heart_disease_model(df):
    ...
    rf_cv = CrossValidator(estimator=rf, estimatorParamMaps=rf_param_grid, evaluator=auc_evaluator, numFolds=5)
    gbt_cv = CrossValidator(estimator=gbt, estimatorParamMaps=gbt_param_grid, evaluator=auc_evaluator, numFolds=5)
    ...
    print(f"Gradient Boosted Trees - AUC: {gbt_auc:.4f}, F1: {gbt_f1:.4f}")
    ...
    final_auc = auc_evaluator.evaluate(final_pred)
    final_f1 = f1_evaluator.evaluate(final_pred)
    print(f"Final Model - AUC: {final_auc:.4f}, F1: {final_f1:.4f}")
    
    # Return the final model, classification report, and ROC-AUC score
    classification_report = f"Final Model - AUC: {final_auc:.4f}, F1: {final_f1:.4f}"
...
def main():
    ...
    smoking_data_two = impute_smoking_2(smoking_data_one)
